...
console.log("[18:01:12] Initializing Command Prompt...");
...
function showDocs() {
    ...
    console.log("[18:01:20] # Hướng dẫn Command Prompt");
...
function startConsole() {
    ...
    const askCommand = () => {
        rl.question('u0@termux:~$ ', async (input) => {
            const command = input.trim();

            if (command === 'gh auth login' || command === 'login') {
                console.log("[18:01:33] u0@termux:~$ gh auth login");
